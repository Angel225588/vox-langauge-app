/**
 * Vocabulary Tab - Word Bank Dashboard (Redesigned)
 *
 * Primary purpose: Help users PRACTICE their vocabulary, not just view it.
 *
 * Key sections:
...
// Existing components we still use
...
export default function VocabularyScreen() {
  ...
  const handleProgressCategoryPress = useCallback((category: 'strong' | 'learning' | 'new') => {
    // Filter by mastery range
    setShowSearch(true);
    // Could implement a mastery filter here
  }, []);

  // Navigate to practice
  const handlePracticePress = useCallback(() => {
    // Navigate to vocab practice with due words
    router.push('/vocab-practice/review' as any);
  }, [router]);

  // Navigate to focus words practice
  const handleFocusWordsPractice = useCallback(() => {
    // Navigate to practice weak words
  ...
  const handleSeeAllFocusWords = useCallback(() => {
    setShowSearch(true);
    // Could implement a weak words filter
  }, []);
